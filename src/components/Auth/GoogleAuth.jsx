...
const googleLogin = useGoogleLogin({
  ...
  return (
    <div className='main__div h-[100vh] flex'>
        <div className="left__side h-[100%] w-[49.9%] flex flex-col flex-shrink-0 bg-gradient-to-b from-[#4285F4] to-[#286DE0] justify-between xl:flex xl:items-center xl:justify-center xl:m-0 xl:h-[60%] sm:h-[30%] sm:flex sm:items-center sm:justify-center sm:m-0 xs:h-[30%] xs:flex xs:items-center xs:justify-center xs:m-0 ">
            <div className='w-69 h-29 ml-[4.562rem] mt-[3.738rem] flex items-center sm:ml-0 xl:ml-0 xs:ml-0'>
                <h4 className='text-white font-poppins text-[1.5rem] font-extrabold leading-29 tracking-tighter text-left sm:text-[20px] xs:text-[15px]'>LOGO</h4>
            </div>
            <div className='ml-[10.688rem] xl:m-0 sm:m-0 xs:m-0'>
                <h1 className='text-white font-montserrat text-7xl font-bold leading-88 tracking-normal text-left sm:text-[30px] xs:text-[20px]'>Board</h1>
            </div>
            <div className='ml-[9.031rem] flex flex-row w-[18.688rem] h-[3rem] justify-between items-center mb-[4.188rem] xl:m-0 sm:m-0 sm:mt-[5px] sm:w-[14rem] xs:m-0 xs:w-[10rem]'>
                <div>
                ...
                <div>
                <div dangerouslySetInnerHTML={{ __html: discordsvg }}/>
                </div>
            </div>
        </div>
        <div className="right__side pl-[5.824rem] bg-[#F8FAFF] w-[100%] flex flex-column justify-center sm:pl-0 xl:pl-0 sm:flex sm:items-center sm:justify-center xs:flex xs:items-center xs:justify-center xs:pl-0 ">
          <div>
            <h1 className='font-montserrat text-3xl font-bold text-left'>Sign In</h1>
            <h4 className='text-black font-lato text-base font-normal pt-[0.343rem]'>Sign in to your account</h4>
          </div>
          <div className='flex mt-[1.784rem] mb-[1.784rem] sm:block xs:block'>
            <div>
            <button
                  onClick={() => googleLogin()}
                  className="relative w-[197.596px] h-[32.933px] bg-white text-[#858585]"
               >
                <div className='absolute top-[8.78px] ml-[20.86px]' dangerouslySetInnerHTML={{ __html: googlesvg }}/>
                <span className="w-100 text-[12px] absolute bottom-[7.68px] ml-[-68.06px]">Sign in with Google</span>
              </button>
            </div>
            ...
            <button
              ...
              className="relative w-[197.596px] h-[32.933px] bg-white text-[#858585]"
            >
            <div className='absolute top-[8.78px] ml-[20.86px]' dangerouslySetInnerHTML={{ __html: applesvg }}/>
            <span className="w-100 text-[12px] absolute bottom-[7.68px] ml-[-68.06px]">Sign in with Apple</span>
            </button>
            </div>
          </div>
          <div className='flex flex-column w-[423px] h-[348px] items-center sm:w-[80%] xs:w-[70%]'>
            <div className='p-[2rem] bg-white flex flex-column flex-shrink-0 w-full'>
            <label className='text-black font-lato text-[1rem] font-normal '>Email address</label>
            <input type="email" className='rounded-sm p-[0.686rem] font-lato bg-[#F5F5F5] mb-[1.373rem]'/>
            <label className='text-black font-lato text-[1rem] font-normal '>Password</label>
            <input type="password" className='rounded-sm p-[0.686rem] font-lato bg-[#F5F5F5] mb-[1.373rem]'/>
            <h3 className='font-lato font-normal text-[#346BD4] mb-[1.373rem]'>Forgot Password?</h3>
            <button className="bg-[#4285F4] text-white p-[0.637rem]">Sign In</button>
          </div>
          <h3 className=' xs:text-[15px]'>Don't have an account? <span className='text-[#346BD4]'>Register here</span></h3>
        </div>
        </div>
    </div>
  )
